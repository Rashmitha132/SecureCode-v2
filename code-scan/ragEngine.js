...
async function askSecurityCopilot({
  ...
  const isBuildOrArchQuery =
    ...
    /\b(how\s*(u|you)\s*(were|was|are|r)\s*(built|made|created|trained|developed))\b/i.test(cleanQuery) ||
    /\b(what\s*(was|is|are|were)\s*(the\s+things\s+)?(used\s+to\s+build|used\s+to\s+train|used\s+to\s+create)\s*(u|you|this))\b/i.test(cleanQuery) ||
    /how u were built|how you were built|how were u built|how were you built|used to build u|used to build you|architecture|what model|which model|dataset|gnn|how does it work/i.test(cleanQuery);
